class NexaFilter {
  ...
  applyFilter(value, filterName, args = []) {
    switch (filterName.toLowerCase()) {
      ...
      case "default":
        return value === null || value === undefined ? args[0] || "" : value;
      default:
        return value;
    }
  }
  ...
  /**
   * Decode URL-safe Base64 to original data
   */
  decode(argument) {
    // Handle null, undefined, or non-string inputs by converting them to strings
    if (argument === null || argument === undefined) {
      return argument; // Return the original value for null/undefined
    }

    // Convert non-string arguments to string
    const stringArg =
      typeof argument !== "string" ? String(argument) : argument;

    try {
      // Konversi ke format Base64 standar
      const paddedString = stringArg.replace(/-/g, "+").replace(/_/g, "/");
      ...
      // Cek jika string terlihat seperti JSON
      if (
        decodedString.trim().startsWith("{") ||
        decodedString.trim().startsWith("[")
      ) {
        try {
          ...
          // Mengembalikan string mentah jika parsing JSON gagal
          return decodedString;
        }
      } else {
        // Langsung kembalikan string jika bukan format JSON
        return decodedString;
      }
    } catch (error) {
