...
export class NexaDom {
  ...
  constructor(options) {
    ...
    // Function to find the element with retry logic
    const findElement = () => {
      // Try different selector strategies
      let element = document.getElementById(elementId);

      // If not found by ID, try with other selectors if provided
      if (!element && options.elementSelector) {
    ...
    // Check if the element exists, if not, try to wait for it
    let oldElement = findElement();

    // Add a small wait before giving up if element isn't found immediately
    if (!oldElement && document.readyState !== "complete") {
      console.log(
        `Waiting for element with ID ${elementId} to be available...`
      );

      // Delay initialization a bit to let DOM catch up
      const checkInterval = setInterval(() => {
        oldElement = findElement();

        if (oldElement || document.readyState === "complete") {
          clearInterval(checkInterval);
          this._initializeAfterElement(oldElement, elementId, options);
        }
      ...
      setTimeout(() => {
        ...
        if (!oldElement) {
          // console.warn(
          //   `Element dengan ID ${elementId} tidak ditemukan setelah menunggu. Membuat element baru.`
          // );
          this._initializeAfterElement(null, elementId, options);
        }
      }, 3000);

      return; // Exit constructor early, initialization will continue in _initializeAfterElement
    }

    // If we get here, either element was found immediately or document is already complete
    this._initializeAfterElement(oldElement, elementId, options);
  }

  /**
   * Continue initialization after element is found or timeout
   * @private
   */
  _initializeAfterElement(oldElement, elementId, options) {
    // Create the element if it doesn't exist
    if (!oldElement) {
      // console.warn(
      //   `Element dengan ID ${elementId} tidak ditemukan. Membuat element baru.`
      // );
      oldElement = document.createElement("div");
      oldElement.id = elementId;

      // Add the element to the document body or a specified container
    ...
    const extractor = oldElement.getAttribute("NexaDom") || elementId;
...
export class NexaDomextractor {
  constructor(filter) {
          ...
          result = `<div class="nx-row">
            ${items
              .map((item, index) => {
                ...
                // TAMBAHKAN KODE UNTUK IF-ELSE BLOCKS
                // Pre-process template untuk menangani blok if-else
                itemTemplate = this.processIfElseBlocks(
                  itemTemplate,
                  item,
                  key
                );

                // Proses template normal (placeholder yang tersisa)
                const fullPattern = /{([^}]+)}/g;
                ...
                itemTemplate = itemTemplate.replace(
                  ...
                  (match, propPath) => {
                    try {
                      // CASE 1: Periksa apakah ini ekspresi kondisional (ternary)
                      if (propPath.includes("?") && propPath.includes(":")) {
                        // Evaluasi ekspresi kondisional
                        // Ekstrak parameter dari item untuk digunakan dalam ekspresi
                        const context = {};

                        // Tambahkan semua properti item ke context
                        Object.keys(item).forEach((propKey) => {
                          context[propKey] = item[propKey];
                        });

                        // Tambahkan alias untuk row dan key
                        context.row = item;
                        context[key] = item;

                        // Coba evaluasi ekspresi ternary
                        try {
                          // Ganti references ke row dan key dengan context
                          let evalExpression = propPath
                            .replace(new RegExp(`${key}\\.`, "g"), "context.")
                            .replace(/row\./g, "context.");
                      ...
                      // CASE 2: Periksa apakah ini operator OR
                      else if (propPath.includes("||")) {
                        const parts = propPath.split("||").map((p) => p.trim());

                        // Coba setiap bagian secara berurutan
                        for (const part of parts) {
                          let value;
                          ...
                          if (
                            ...
                            const cleanPath = part
                              .replace(`${key}.`, "")
                              .replace("row.", "");
                            value = getNestedValue(item, cleanPath);
                          } else {
                            value = item[part];
                          }

                          // Jika nilai valid, gunakan
                          if (
                            value !== undefined &&
                            value !== null &&
                            value !== ""
                      ...
                      // CASE 3: Periksa untuk filter (menggunakan pipe |)
                      else if (propPath.includes("|")) {
                        const [path, filters] = propPath.split("|", 2);
                        let value;

                        // Dapatkan nilai dasar
                        if (
                          path.startsWith(`${key}.`) ||
                          path.startsWith("row.")
                        ) {
                        ...
                        if (filters && this.filter) {
                          ...
                          filtersList.forEach((filterStr) => {
                            ...
                            if (typeof this.filter.applyFilter === "function") {
                      ...
                      // CASE 4: Penanganan properti normal
                      else if (
                        ...
                        const cleanPath = propPath
                          .replace(`${key}.`, "")
                          .replace("row.", "");
                        const value = getNestedValue(item, cleanPath);
                        return value !== undefined ? value : "";
    ...
    // Tambahkan method baru untuk memproses blok if-else
    this.processIfElseBlocks = function (template, item, keyName) {
      // Pola untuk mendeteksi blok if-else
      // Contoh: {if(row.type === 'dir') { <p>Folder</p> } else { <p>Icon</p> }}
      const ifElsePattern =
        /{if\s*\(([^)]+)\)\s*{([^}]*)}\s*else\s*{([^}]*)}\s*}/g;

      // Contoh lain: {if(row.type === 'dir') { <p>Folder</p> }}
      const ifPattern = /{if\s*\(([^)]+)\)\s*{([^}]*)}\s*}/g;

      // Buat context untuk evaluasi
      const context = { ...item, row: item, [keyName]: item };
      ...
      let processedTemplate = template.replace(
        ...
        (match, condition, ifContent, elseContent) => {
          try {
            // Persiapkan kondisi untuk evaluasi
            const preparedCondition = condition
              .replace(new RegExp(`${keyName}\\.`, "g"), "context.")
              .replace(/row\./g, "context.");
            ...
            // Kembalikan blok yang sesuai
            return conditionResult ? ifContent.trim() : elseContent.trim();
          } catch (error) {
            console.warn("Error dalam evaluasi if-else:", error, condition);
            return match; // Pertahankan isi asli jika terjadi error
          }
        }
      );

      // Proses blok if saja (tanpa else)
      processedTemplate = processedTemplate.replace(
        ...
        (match, condition, content) => {
          try {
            ...
            // Kembalikan konten jika kondisi true, atau string kosong jika false
            return conditionResult ? content.trim() : "";
          } catch (error) {
            console.warn("Error dalam evaluasi if:", error, condition);
            return match; // Pertahankan isi asli jika terjadi error
          }
        }
      );
